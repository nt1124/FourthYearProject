...
void isaac_randinit(randctx *ctx, word flag)
{
	 word i;
	 ub4 a,b,c,d,e,f,g,h;
	 ub4 *m,*r;
...
ub4 *getIsaacContext(randctx *ctx)
{
...
int ISAAC_main()
{
	unsigned char *temp = (unsigned char *) calloc(32, sizeof(unsigned char));
	ub4 i,j, tempUL;
	randctx ctx;
	...
	for (i=0; i<2; ++i)
	{
		...
		for (j=0; j<256; ++j)
		{
			tempUL = isacc_rand(&ctx);
			memcpy(temp, &tempUL, sizeof(ub4));
