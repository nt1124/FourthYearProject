...
\begin{document}
	...
	\begin{titlepage}
		\begin{center}
			...
			\vspace{3cm}
			\HRule \\[0.4cm]
			\textsc{ \huge \bfseries Secure Two Party Computation - A comparison of recent protocols}
			\HRule \\[0.4cm]
			\vspace{2cm}
    
			\begin{abstract}
				We shall be producing implementations of several proposed Secure Two Party Computation protocols, several of which till now unimplemented, with a view to comparison.\\
				
				These protocols include the one put forward by Prof. Lindell in \cite{Lindell_CnC_2014}.
			\end{abstract}
	...
	\section{Introduction}
		\subsection{Problem definition} \label{InformalProbDef}
			...
			Some time should be taken at this point to note also that we are pursuing a generalised solution to this problem. Many SMC protocols exist that are designed for a specific problem, whilst these are often more efficient than any general solution they are limited to one problem. We will be focusing on general protocols that can be applied to any computation.
