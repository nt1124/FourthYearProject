...
\begin{document}
	...
	\chapter{Introduction}
		...
		\begin{itemize}
			...
			\item We also provide an implementation of the protocol from \cite{Katz_Symm_CnC_2013}.
			\item We put forward and implement a modification of \cite{Lindell_CnC_2013} using our implementation of \cite{Katz_Symm_CnC_2013} for the sub-computation rather than \cite{LindellAndPinkas2011} as originally proposed.
	...
	\chapter{Background to Secure Multiparty Computation} \label{sec:BG_toSMC}
		\section{Security Properties} \label{sub:SecurityProperties}
			...
			It is worth noting that some functions inherently leak information about the inputs of the other parties. For example in a two party addition both parties can easily recover the other's input after the computation has been run by subtracting their own input from the result. In these cases SMC is not at fault.\\
		...
		\section{Security levels}\label{sub:SecurityLevels}
			...
			\subsection{Covert Adversary}
				...
				\subsubsection{Example} 
					...
					{This model can be thought of as a compromise between practicality and malicious security and is usually appropriate when there are tangible consequences to a party being caught cheating. For example consider a consortium of companies who wish to cooperate in some way that benefits participents and that if one is caught cheating in the computations they are publically expelled from the consortium.\\}
	...
	\section{Yao's Protocol} \label{sec:Yao_Circuits}
		...
		\subsection{Yao Garbled Circuits} \label{sub:Yao_Details}
			...
			\subsubsection{Free XOR Improvement}
				...
				This is done by introducing a relationship between the 0-key($k_0$) and 1-key($k_1$) for each wire. In particular an $R$ is chosen at random for each Yao Garbled Circuit and then whilst $k_0$ is generated randomly as usual we take $k_1 := k_0 \oplus R$.
	...
	\chapter{Protocols} \label{sec:Protocols}
		...
		\section{Lindell 2013}
			\subsection*{Overview}
				...
				\subsubsection{Secure Computation to detect cheating}
					...
					Then if any of the circuits evaluated by the Executor give different outputs on any output wire (say output wire $i$) the executor then obtains both $b_i^0$ and $b_i^1$, these will then be used as input to the cheating detection. If all circuits produce the same output then the Executor randomly generates this input.\\
					
					The parties then perform a Secure Computation where the Builder inputs $x$ and the Executor inputs $b$.
		...
		\section{Huang, Katz and Evans 2013}
			\subsection*{Overview}
				...
				The observant reader might question what one does if a party gets both $0$ and $1$ on some output wire in different circuits, and the same occurs for our partner. This situation is only possible if both parties cheated, in which case we care little for their plight. If at least one party is honest then this party will provide honest circuits. As such at least one party will have the correct value for every output wire in all their evaluation circuit.\\
	...
	\chapter{Implementation Details} \label{sec:ImplementationDetails}
		...
		\section{Elliptic Curve implementation}
			...
			Take $P = (x_1, y_1)$ and $Q = (x_2, y_2)$, then $(x_3, y_3) = P + Q$ is as follows\\

			$(x_3, y_3) = (@, @) \textnormal{ if } x_1 = x_2 \textnormal{ and } y_1 \neq y_2$\\[0.1cm]

			$x_3 = (\frac{y_2 - y_1}{x_2 - x_1})^2 - x_1 - x_2 \textnormal{ and }$
			$y_3 = (x_1 - x_3) * \frac{y_2 - y_1}{x_2 - x_1} \textnormal{ where } x_1 \neq x_2$
	...
	\chapter{Experiments} \label{sec:Results}
		...
		\section{Results}
			...
			\subsection{32-bit multiplication}
				...
				The 32-bit multiplication circuit is significantly larger than addition yet smaller than AES, providing a good mid-way stepping stone to the AES circuit.
			...
			\subsection{SHA-256 Hashing}
				...
				This test should heavily favour Lindell's 2013 protocol over the Lindell-Pinkas protocol from 2010 due to the size of the circuit. Of particular interest will be to see how the HKE protocol fares compared both variants of Lindell's 2013 protocol.\\
	...
	\begin{appendices}
		...
		% 				\end{figure}
			...
			\section{Elliptic Curve Group Operations}
				...
				We benchmark point addition, point multiplication and fixed point multiplication. The fixed point multiplication includes the pre-computation of the relevant windows.

			\section{Oblivious Transfer}
				We benchmark all the Oblivious transfers we use, in each case we include the setup of the OTs in the measurements and we also state the communication costs (number of bytes exchanged).
			...
			\section{Circuit Evaluation}
				...
				Once a party has the inputs for a Yao Garbled Circuit the circuit must be evaluated. We show benchmarks for all the circuit we shall be testing.
