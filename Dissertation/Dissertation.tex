			...
			\section{Comparison of protocols}
				...
				\subsection{Comparison of sub-computations}
					...
					\noindent \textbf{Comparison}\\
						...
						That said at present I can think of no reason this optimisation cannot be applied to the Lindell protocol directly, though it would require committing to the inputs as in the HKE protocol. If this can be done then the key advantage of the L-HKE protocol over large input circuits is nullified.\\

						We conclude that while the L-HKE sub-computation shows promise it needs improvement before it can compete with the Lindell sub-computation on large circuits. It's primary problem is its sensitivity to variations in the Builder's input size.


	\chapter{Conclusions}
		Our work provides the first practical implementation of two protocols, a new variant on one of those protocols and an implementation of a protocol for which only preliminary experiments have been performed. As such we provide the first practical comparison of these four protocols.\\

		As important as theoretical comparisons are they can only tell us so much. For example in \cite{Lindell_CnC_2013} Lindell explicitly compares the Lindell 2013 protocol with the Lindell-Pinkas 2010 protocol. He notes how the L-2013 protocol trades off more exponentiations for fewer symmetric encryptions and less bandwidth usage. However this theoretical comparison does not tell us if this trade off is worth it.\\
		...
		\section{Future Work}
			...
			Additionally there are some corner cases we have not addresses, in particular the case where one party has input size zero. This is a somewhat dubious corner case though as if a party has no inputs to the computation there is little reason for a secure computation.\\
			...
			Whilst we only had limited success with modifying the sub-computation of the Lindell-2013 protocol the results we obtained were promising. Further work could be done into the modified version of the Lindell-2013 protocol that we put forward, in particular into the problem of hiding the output of the sub-computation in a way that does not inflate the input size.\\
