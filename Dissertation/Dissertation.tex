...
% 					In short, both parties could potentially craft a malicious $crs$ during setup allowing them to subvert the properties of the Dual-mode system, so the solution is to divide the work between the two parties so each provide some of the $crs$ in such a way that neither has enough information to create the trapdoor that is worth anything to them .
	...
	\section{Yao's Protocol} \label{sec:Yao_Circuits}
		...
		\subsection{Yao Garbled Circuits} \label{sub:Yao_Details}
			...
			$P_1$ also generates a random permutation $\pi_i \in \{0, 1\}$ for each $w_i$. We define $c_i = \pi_i(b_i)$. The garbled value of the $i^{th}$ wire is then $k_i^{b_i} \Vert c_i$, we then represent our garbled truth table for the gate with the table indexed by the values for the $c_1$ and $c_2$.
			...
			Suppose that $AES_k(m)$ denotes the AES encryption of the plaintext $m$ under the 128 bit key $k$ and $AES^{-1}_k(c)$ denotes the decryption of ciphertext $c$ under key $k$. We define $E_K$ (and it's inverse $D_K$) as follows,
...
% 
				\begin{figure}[!ht]
					\begin{center}
						...
						\end{tabular}
					\end{center}
					\caption{A tabulation of an XOR gate. \label{table:FREE_XOR_Demonstration}}
				\end{figure}
	...
	\chapter{Summary of Protocols to be implemented} \label{sec:Protocols}
		...
		\section{Lindell and Pinkas 2011}
			\subsection{Overview}
				...
				Firstly this protocol gives an improved deterrent probability of $\epsilon = 1 - 2^{-0.311 S}$, further the work in \cite{ShelatAndShen} showed how to achieve a slightly improve deterrent probability of $\epsilon = 1 - 2^{-0.32 S}$.\\

				Secondly it removes the need for the very large number of commitments entailed in \cite{LindellAndPinkas2007}, this is particularly of note as this was one of the main costs in the Lindell-Pinkas-Smart implementation of the previous protocol \cite{LindellPinkasSmart2008}. Indeed Lindell and Pinkas comment in their introduction to this protocol that the previous protocol, when running on a circuit with input size $128$ required $6,553,600$ commitments.\\

				Thirdly it does not require the preprocessing of the circuit that vastly inflates the number of input wires for the Executor and thus the number of Oblivious transfers needed.
			...
			\subsection{Consistency of building party's inputs}
				...
				The builder then computes $\{g^{a_i^0}, g^{a_i^1}\}_{i = 1}^{l}$ and $\{g^{r_j}\}_{j = 1}^{S}$ and uses $K_0 = H(g^{r_j^{a_i^0}})$ as the $0$-key on the $i^th$ Builder input wire in the $j^{th}$ circuit. Similarly he uses $K_1 = H(g^{r_j^{a_i^1}})$ as the $1$-key on the same wire. Lindell and Pinkas note this means the keys are actually \emph{Pseudo-Random Synthesizers} \cite{PseudoRandomSynth}, meaning if some of the keys are revealed (a necessity for checking correctness of circuits) the rest remain pseudo random.\\
		...
		\section{Lindell 2013}
			...
			\subsection{Secure Computation to detect cheating}
				...
				\subsubsection{The Oblivious Transfer Optimisation}
					...
					Then take the keys for this input wire for circuit to be $\{k_0^j, k_1^j\}$. All the $k_0^j$ can be simply sent to the Executor. Then for each circuit $j$ construct the set $\{Y_i^j\}_{i = 1}^{S}$ such that $Y_1^j \oplus ... \oplus Y_S^j = k_1^j$.\\
	...
	\chapter{Experiments} \label{sec:Results}
		...
		\section{Expectations}
			...
			\subsection{32-bit addition}
				...
				We expected to see a poor showing from the Lindell 2013 protocol due to the circuits small size increasing the relative cost of the sub-computation. Due to the relatively high input wire to gate ratio we expect the cost of Oblivious Transfers to be fairly high.
			...
			\subsection{32-bit multiplication}
				...
				Furthermore, as the inputs sizes are the same for both parties the number of OTs is the same as in the Addition circuit, meaning we get so see how much importance the `depth' (size of circuit discounting inputs) of the circuit has with regards to performance. We expect to see the costs of OTs to remain the same and for the circuit building/checking times to be marginally affected compared to the addition circuit.\\

				At first one might be surprised by this second expectation, after all the multiplication circuit has around $30$ times more gates. But consider this, the main cost of building circuit once inputs/outputs are generated are the Symmetric encryptions for create the encrypted output tables. With AES-NI these costs are reduced to such a degree that the input/output generation dominates the cost of building circuits.
			...
			\subsection{AES encryption}
				...
				We considered experimenting with the alternative circuit provided by \cite{NigelCircuits} which takes in an expanded key schedule reducing the circuit size in exchange for an increase in the number of inputs by a factor of $10$ for the Executor. However, while the reduction in circuit size would reduce building, checking and evaluation time this would be more than nullified by the increase in the number of inputs, not only in terms of OTs but also in terms of generating consistent inputs.
		...
		\section{Results}
			...
			\subsection{32-bit addition}
				...
				\begin{figure}[!ht]
					\begin{tabular}{| p{3.5cm} | c c c c |}
						\hline
						\textbf{L-HKE 2015} & & \textbf{Builder} & & \\
						\hline
						\textbf{Step} & \textbf{CPU Time} & \textbf{Wall Time} & \textbf{Bytes Sent} & \textbf{Bytes Received} \\
						\thickhline
				...
				\begin{figure}[!ht]
					\begin{tabular}{| p{3.5cm} | c c c c |}
						\hline
						\textbf{L-HKE 2015} & & \textbf{Executor} & & \\
						\hline
						\textbf{Step} & \textbf{CPU Time} & \textbf{Wall Time} & \textbf{Bytes Sent} & \textbf{Bytes Received} \\
						\thickhline
