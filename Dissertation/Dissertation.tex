	...
	\chapter{Summary of Protocols to be implemented} \label{sec:Protocols}
		...
		\section{Lindell 2013}
			...
			\subsection{Secure Computation to detect cheating}
				...
				The parties then perform a Secure Computation to detect cheating (here on in, the Sub-computation) where the Builder inputs $x$ (its original input to the main computation) and the Executor inputs $b$. The Secure Computation returns $x$ to the Executor if its input $b$ indicates it knows both $b_i^0$ and $b_i^1$ for some $i$, otherwise it returns garbage.\\
