...
struct commit_pair_Boxes *init_Pair_Boxes()
{
	struct commit_pair_Boxes *toReturn = (struct commit_pair_Boxes*) calloc(1, sizeof(struct commit_pair_Boxes));

	toReturn -> Box_0 = init_commit_box();
	toReturn -> Box_1 = init_commit_box();

	return toReturn;
}

void free_Pair_Box(struct commit_pair_Boxes *toFree)
{
	free(toFree -> Box_0);
	free(toFree -> Box_1);
...
struct commit_pair_Keys *init_Pair_Keys()
{
	struct commit_pair_Keys *toReturn = (struct commit_pair_Keys*) calloc(1, sizeof(struct commit_pair_Keys));

	toReturn -> Key_0 = init_commit_key();
	toReturn -> Key_1 = init_commit_key();

	return toReturn;
}

void free_Pair_Key(struct commit_pair_Keys *toFree)
{
	free(toFree -> Key_0);
	free(toFree -> Key_1);
